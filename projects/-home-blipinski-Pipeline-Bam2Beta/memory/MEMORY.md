...
- [QC post-filtre 80 pb – 1 kb](qc-filtre-80-1kb.md) — R&D 1378 liquides : **une passe streaming** `aws s3 cp -|samtools|mawk`, conventions validées **bit à bit** sur Lung_9 (bases = **M/=/X seuls**, mosdepth ignore les délétions ; dénominateur **3 099 721 093**). Le `read_lengths.csv` ne suffisait pas (**ni positions ni strate A**). Le filtre coupe du **court** (écartés 78,5 pb vs 177,2 conservés) → comptages −7,4 % > depth −3,2 % > coverage −0,96 %. Traîne du depth = **détecteur de gDNA** (facteur 30 vs masse > 1 kb, `TNE_2` 0,66→0,13×). Bascules QC : **50 Exis / 46 Themelio**, toujours par le comptage de molécules, jamais par depth/coverage ; Themelio bascule **moins** car déjà FAILED sur l'amplitude. ⚠⚠ **16 flux = optimum, 32 s'effondre** (96 Go RAM, débit ÷6) ; télécharger le BAM avant est **plus lent** que le streaming ; mawk ne gagne que 16 %. ⚠ `qc_metrics.coverage_percent` n'a qu'**1 point de précision** (entier rond 1378/1378)
