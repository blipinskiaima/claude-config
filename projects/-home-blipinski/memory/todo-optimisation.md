...
- [ ] **Aima-Tower — CUP balanced accuracy +8,4 pt inexpliqué** — 72,9 % publié contre 81,3 % mesuré, le plus gros écart de `/qara`. Recalculer les rappels par classe des deux côtés pour identifier quelle classe bouge. La cohorte passe aussi de 284 (jeu de développement figé au moment de l'entraînement) à 522, ce qui peut suffire à l'expliquer — ou pas. Relevé le 2026-09-11.

- [ ] **`MEMORY.md` d'Aima-Tower dépasse sa limite de chargement** — 29,6 Ko pour ~24,4 Ko lus au démarrage : les entrées les plus anciennes ne sont plus en contexte. Consolider les entrées d'index, devenues des paragraphes. Signalé deux sessions de suite.
...
## 2026-09-11 — Aima-Tower : charte graphique du site officiel (v5.6.0) · trace-prod : schema v32 threshold, lot clos · Bam2Beta : où passe le temps dans EXIS + right-sizing des ressources · Bam2Beta + trace-prod : QC post-filtre 80 pb – 1 kb (v35) · /qara comparaison au point figé (v5.7.0)
- [x] **Aima-Tower — comparaison au point figé sur `/qara` (v5.7.0)** — un interrupteur déplie une seconde rangée mesurée sur trace-prod, alignée sur les cartes figées par réutilisation du composant `Kpi`. Trois régimes assumés : **Exis** sans calcul maison (un seul écart, `Prostate_21`), **CUP** par comptage de `too_final_decision` (**284/284 mVAF identiques**), **Themelio** mesuré mais non comparable — le doc publie de l'out-of-fold, la base du score de production, 54,5 % contre 62,3 % avec le même bundle des deux côtés. **Divergence 94/95 du doc CUP résolue** : un échantillon a `max_p` exactement au seuil, le `>` strict donne les chiffres publiés. Commit `ccfdaf7`, détails dans `~/.claude/projects/-home-blipinski-Pipeline-Aima-Tower/memory/qara_comparaison_dynamique.md`.
...
- [x] **trace-prod — schema v32 `rarefaction_horaire_threshold` (lot clos)** — table autonome 63 col, PK composite `(sample_name, labo)` posée d'emblée grâce aux **28 collisions inter-labo repérées avant le DDL** (le bug v17 avait coûté 90 lignes) ; `mvaf_v1`/`v2` retirés, leur `raima_score.V2.tsv` n'existe dans aucun dossier. Alimentée en **9 passes** du 03 au 09/09 (308 → **1620 lignes**, 405 bases × 4 paliers), **PROD/mVAF/epic/Loyfer à 1620/1620**, 2 onglets exportés avec 0 cellule divergente. Zéro duplication de code (imports `_bootstrap_means`/`_read_props`, helper d'export réutilisé). Commits `634afa5` + `2444113`, mémoire `project_schema_v32_rarefaction_horaire_threshold.md`.
- [x] **Temps 2 de l'expérience de concordance — récapitulatif par seuil livré** — les scores existent pour les **1620 pseudo-samples** (mVAF v1.4/v1.5 + 47 probs), et l'agrégation par palier est l'onglet `mVAF` de la gsheet `1FG4KfL4…` : une ligne par `{base}_{labo}_{palier}`, valeurs initiales et threshold accolées par paires. ⚠ Fait **côté trace-prod uniquement** — la boucle `--EXIS true --MERGE false` n'a pas été écrite ni versionnée dans Bam2Beta ; à rouvrir si le cahier des charges l'exigeait comme script.
- [x] **Piège retenu : un BAM re-sous-échantillonné rend son aval périmé** — relancer une base réécrit les 4 BAM ; si l'aval (QC → BETA_28M → mVAF/BOOTSTRAP/Loyfer) ne suit pas, les valeurs en base viennent d'un **autre tirage**, et **trace-prod ne peut pas le détecter** (il lit des fichiers, pas leurs dates relatives). Le cas dangereux est celui **sans aucun `NA`** : `Lung_77_20M` affichait 12 valeurs sur 12, toutes périmées, et reste à reprendre. Détection = date S3 du `BAM/` vs du `BOOTSTRAP/`. Vaut pour toute table nourrie de fichiers régénérables (`rarefaction`, `dilution`, `dilution_lung`).

- [x] **Bam2Beta — où passe le temps dans EXIS, mesuré sur 46 traces** — `Raima_score_mVAF` pèse **74-81 % du wall-clock EXIS** sur 7 samples de 0,4 à 44 Go et sur deux architectures, avec une **queue mono-tâche à 12 % d'occupation machine** sur 30-62 % du run ; la phase MERGE, toujours mono-sample, fait **40 %** du run total. Source non évidente : les lanceurs passent `-with-trace` en CLI malgré `trace.enabled=false`, donc **chaque run prod dépose sa trace sur S3** dans `{sample}/LOG/` — `cleanup=true` ayant purgé le workDir local. Mémoire `perf-exis-traces.md`.
- [x] **Bam2Beta — right-sizing des ressources sur ~5 500 tâches** — `IV_call` 8→16 G (**2 `exit 137` réels** corrigés), ternaire `Mosdepth_qc` rendu cohérent à 16 G (le « corriger » à 8 aurait tué **54 %** des tâches mosdepth), ~20 sur-allocations ramenées à 2 G, phase MERGE montée à 16 cpus (`BAM_sort` 40 G) après avoir prouvé que `samtools sort` est déterministe entre nombres de threads. Commits `0856f3f` + `bbbc009`, mémoire `ressources-dimensionnement.md`.
- [x] **Piège retenu : `peak_rss` est censuré par le plafond cgroup** — aucune des 5 500 tâches n'excède jamais son allocation, donc un ratio faible **prouve** la sur-allocation mais un ratio ~100 % est **indécidable** (bien dimensionné ou sous pression). Corollaire : Nextflow passe `--memory` (plafond dur, OOM 137) mais `--cpu-shares` (simple poids, aucun plafond) — les scripts R CNV consomment 5,5-7 cœurs pour 2 déclarés.
...
- [x] **trace-prod — agrégats par run corrigés : rebasecallés exclus, molécules au lieu des lignes** — `reads_per_flowcell` sommait les rebasecalls (sur `9fc13cc1`, `Healthy_23/25/26` comptés **5 fois chacun** → 876,15 M stockés pour 238,13 M réels, le maximum de la colonne). Désormais hors rebasecallés (qui reçoivent NULL) et en `nb_reads_aligned`. **1515 valeurs recalculées, 208 passées à NULL**, max liquid 876,15 → 335,92 ; le solid ne bouge que par le changement de métrique (0 rebasecallé sur 147). Base ↔ export `run` : 291/291 identiques. Les appelants (`Bam2Beta.sh`, `launch.sh`) en bénéficient sans modification. Commit `e4d0873`.
- [x] **trace-prod — renommage des colonnes `nb_lignes_total` / `nb_molecule` (migration v34)** — noms **techniques** des colonnes DuckDB alignés sur les étiquettes de septembre (14 colonnes, 7 tables ; étiquettes d'export inchangées). ⚠⚠ **Un sed global réécrit l'intérieur de la migration elle-même** (`LIKE 'nb_reads_total%'` → `LIKE 'nb_lignes_total%'`) et la rend **no-op silencieux** — aucune erreur, base jamais migrée ; détecté par le contrôle de non-régression, pas à l'exécution. Les **noms de fichiers** `{s}.nb_reads_total.tsv` (produits par Bam2Beta) sont protégés. Non-régression : **9275 lignes ligne à ligne, 0 écart**, 19 contraintes identiques, + A/B de `check`/`probs`/`update-column` contre le code d'avant. Commit `19923c1`, mémoire `project_rename_nb_lignes_molecule_cols.md`. ⚠ `SCHEMA_VERSION` reste à **33** : à bumper avec la session qui porte la v33.
- [x] **trace-prod — 5 colonnes masquées de l'export** — `mVAF v1`, `mVAF v2`, `Multi Run`, `BEDMETH EPICS`, `Props Epic` retirées de `_LIQUID_QC`/`_SOLID_QC` (liquid 57 → 52, solid 40 → 37), **mappings conservés** : les colonnes restent alimentées en base, comme `n75` depuis la v22. Commit `ef0af08`.
...
- [x] **Bam2Beta + trace-prod — `sequencing_time` calcule par le pipeline** — `Read_Start_Time` publie `{ID}.sequencing_time.tsv` (duree du run + `multi_run`) dans la passe awk qui extrait deja les `st:Z:`, et `scan_read_start_time(allow_scan=)` le lit au lieu de balayer 2,5 Go/sample (~9h30 sur la cohorte) ; le check de routine remplit desormais les 2 colonnes, **liquid seulement**. ⚠ Le mecanisme de preservation est l'**omission de la clef** dans le dict de `check_sample` — une valeur `'KO'`/`None` aurait ecrase les **1362** valeurs backfillees (`_upsert_table` fait `SET c = EXCLUDED.c` sans skip des `None`). Bug DST mesure **non-declenchant** (0 erreur/1362, `Prostate_31` verifie sur 33 M reads, 2 offsets presents) → **aucun backfill**. Valide de bout en bout sur les 8 Twist `_rep_3` (TSV simules sur S3, base 1362 → 1370). Commits `bb84b5e` + `608fdf3`, memoire `sequencing-time-fastpath.md`.
...
- [x] **Bam2Beta — statut QC Exis/Thémélio dans metadata.json** — arbre de l'onglet Synthèse implémenté en un process `QC_status` (rapport.nf), 4 champs (JSON à 33), validé 19 cas + Healthy_826/Lung_9 ; cohorte : CGFL 449 S / 223 W / 105 W-F / 77 F, HCL 475 / 31 / 6. Détails `qc-status-exis-themelio.md`.
- [x] **Bam2Beta — module rétro `--RETRO_REPORT`** — 1366/1379 metadata.json liquid régénérés depuis les sorties publiées (157 anciens sauvegardés `.pre_qc_status`) ; le glob `RetD/liquid/CGFL/*` casse côté S3 (cause non établie), CGFL généré par Boris via `data/CGFL/liquid/*`.
- [x] **trace-prod — schema v34, 4 colonnes QC lues dans le JSON** — `read_qc_status` preserve, `update-column` une lecture → 4 colonnes, gsheet vérifiée (après mVAF v1.5 / Thémélio, distributions = base = JSON).
- [x] **`sequencing_time.tsv` publié par Read_Start_Time** — même passe awk (mktime), trace-prod le lit d'abord (`allow_scan`) : 2,5 Go/sample → 60 o ; preserve prouvé (Prostate_31), chemin rapide prouvé (Twist_10_6_rep_3) ; lexicographique historique = mktime sur les 28 runs traversant un changement d'heure.
- [x] **rapport.nf simplifié** — 291 → 161 lignes : `cp` mort, awk V2.json mort (qui ne parsait même pas, apostrophes l.204), `csv_to_kv` factorisé ; metadata.json bit-à-bit identique (différentiel + run Healthy_826).
...
- [x] **Aima-Tower — `/profil-aima` sans comparaison chiffrée** — retrait de tout ce qui mettait un de nos chiffres en face d'un chiffre concurrent : colonnes **Eux / Nous / Comparabilité** de la Vue d'ensemble, tableau AIMA/concurrent côte à côte des fiches (→ concurrent seul), écart de sensibilité, badge de degré, point de fonctionnement, encart « Différences de mesure », verdict chiffré. Motif : nos cohortes ne sont pas les leurs, deux colonnes en regard se lisent comme une mesure commune qui n'existe pas. 667 → 462 lignes, 8 composants orphelins supprimés. Backend inchangé (retrait front pur). Commit `09d6a11`, détails : `~/.claude/projects/-home-blipinski-Pipeline-Aima-Tower/memory/profil_aima_sans_comparaison.md`.
- [x] **Aima-Tower — synthèse d'article `/survey` en Opus 5** — la synthèse ne résume pas seulement l'abstract, elle tranche les implications AIMA et émet une recommandation : même critère que la génération de code et de SQL d'`/analytics`. Appel à la demande, caché par PMID, seule la latence double (~39 s mesurées). Commit `f9ca7f7`.

- [x] **short-read — mVAF v1.4/v1.5, protocole établi** — le read-level qu'exige le bootstrap se récupère du BAM avec `rastair per-read`, pas des `CX_report`/`rastair_call` archivés (agrégation irréversible) ; ni tags MM/ML ni modkit, `bootstrap_model_v1()` tourne inchangé. Reformatage validé par un gate qui redonne le `rastair_call` d'origine (couverture 97,4 %, corrélation β **+0,997**) — il a fait échouer 3 versions du repliage de brins avant de passer. Pilote NF_Methylseq, 2 samples : Colon_3 v1 18,03 → v1.4 **42,81** (ratio 2,37, médiane ONT 2,26 sur 1 509 samples), Healthy_634 0,00 → 0,01. Reste le gate version `CX_report`, qui débloque BP_5base et BP_Watchmaker. Commit `6fe8706`, détails : `~/Pipeline/short-read/mvaf14_short_read/` et `project_mvaf14_short_read.md`.

- [x] **Aima-Survey — Biodesix en 8e concurrent surveillé** — entrée tier_3 + bloc `watch` (SEC, ClinicalTrials, page de communiqués), terme `[Affiliation]` dans `queries.json`, dossier complet en 4 volets visible dans l'onglet Deep dive concurrent. P0/P3 générés depuis `competitive_events`, P1/P2 rédigés puis vérifiés en source primaire (API XBRL SEC, API ClinicalTrials.gov, résumés PubMed) — ce qui a corrigé les effectifs de CLARIFY et d'ALTITUDE et révélé la **sensibilité de 9-16 %** que leurs communiqués ne publient jamais. Verdict : ni méthylation ni fragmentomique, menace basse. Commit `3d9effa`, détails : `~/.claude/projects/-home-blipinski-Pipeline-Aima-Survey/memory/biodesix.md`.
- [x] **Aima-Survey — source de veille ClearNote en échec réparée** — la règle anti-domaine-détourné jugeait la liste **fusionnée** des valeurs d'une clé `watch` : un sitemap de publications scientifiques (0 titre sur 21 nommant la société) faisait rejeter le sitemap de communiqués sain qui l'accompagnait (26 sur 78), et toute la newsroom tombait sur un faux « domaine abandonné ou détourné ». Jugement désormais **valeur par valeur** — plus protecteur aussi, un domaine réellement détourné parmi trois sains se cachait derrière ses voisins. `publication-sitemap` retiré du `watch`, le canal PubMed le couvre mieux. 255 tests, 0 source en échec sur les 8 concurrents.
- [x] **Skill `/deep-dive-concurency` — 7 dérives corrigées** — il décrivait la génération de juillet : 2 volets au lieu de 4, `rapports/` au lieu de `profils/` (le seul chemin que la Tower lit), slug par produit au lieu de par société, §0 collé dans les rapports au lieu du volet P0 autonome, fiche intermédiaire abandonnée depuis Natera, `queries.json` absent de la phase d'intégration, et des marqueurs de preuve qu'aucun des 14 rapports n'emploie. Biodesix est le premier dossier produit par le skill corrigé.
...
- [x] **mVAF v1.5 intégrée à Bam2Beta** — v1.4 corrigée par la couverture EPIC (`raima::transfo_mvaf_by_cov`, raima 0.5.4). Voie standard + voie rétro `--MVAF15_RETRO`, `cmp`-identiques sur Healthy_826 ; V1.4 bit-à-bit inchangé. ⚠ `nb_read_epic` se compte en **millions** (sinon `NA` puis plantage), et l'image 0.5.4 est locale, non poussée. Commit `983af28`, détail : `mvaf-v1.5.md`.
- [x] **Batchs rétro débloqués** — `MERGE=false` passe de `checkIfExists` à `filter+warn` : un sample sans BAM merged ne tue plus un run `--input ".../*"` (constaté sur `Bladder_Urine_02_109`).
...
- [x] **Backfill du ratio N50/N75 sur les 38 urines manquantes** — script pérennisé dans `dev/backfill_n50_ratio.sh` (le précédent, jeté dans un scratchpad, avait été perdu) : recalcul depuis le `read_lengths.csv` déjà sur S3, **sans stager le BAM**, validé `cmp`-identique au pipeline sur Healthy_826 + Breast_6 avant usage. 38/38 OK, dont **30 en zone C** (79 %, au-delà des 71,6 % attendus pour de l'urine). Commit `5693a43`.
- [x] **MITO et SMALL_FRAGMENTS rétrospectifs sur les Bladder_Urine** — 37/37 et 7/7. ⚠ **8 runs Nextflow en parallèle saturent `/scratch`** (8 × 21 Go de staging simultané) → 24 échecs `No space left on device` ; tout récupéré en repassant en série. Sur les 55 samples visés en small fragments, **48 étaient déjà faits** (module inchangé depuis `d6d4556`) : croiser avec l'existant avant de lancer, pas après.
- [x] **mVAF v1.5 vérifiée déployée sur tout le liquide** — scan S3 : **1 362/1 362** liquides (849 CGFL + 513 HCL) portent `V1.4` **et** `V1.5`, aucun écart de périmètre entre les deux. Les 147 solid CGFL n'en ont aucun — attendu, `BETA_28M` n'y tourne pas.
- [x] **Schema v27 — renommage `short_read` → `small_fragments`** — le miroir S3 avait été renommé côté pipeline, le checker listait un préfixe mort : la colonne serait passée **KO sur 100 %** des samples au prochain check. Renommé partout (colonne, table + 26 colonnes, CLI, module, onglet). Migration en **pré-migration avant les `CREATE`**, sinon `CREATE IF NOT EXISTS` fabrique une table vide et le `RENAME` échoue. `trace-prod` 63418c0.
- [x] **38 `idxstats.tsv` générés hors Nextflow** — `samtools idxstats` lit le `.bai` (5 s/sample, 3 min en tmux ×8). Débloque `Mapped %` / `Primary mapped %` / `Unmapped %` / `Hors chr1-22 %`, vides jusque-là sur les Bladder_Urine récents.
- [x] **Les 38 urines PROD KO expliquées** — aucun fichier ne manquait : run figé sur `-r V2.2.0` (21/07) alors que le module MITO date du 27/07, contrôlé par un check de conformité local à jour. Verdict périmé, pas échec de calcul. Résolu après relance MITO + recheck → 38 PROD OK.
- [x] **N50 et mVAF v1.3 masqués à l'export** — retirés de `_LIQUID_QC`/`_SOLID_QC`, colonnes et mappings conservés (statut de `n75`). + fallback `Indication` de 'QC read' étendu (`Bladder_Urine%`, `Colon%`) plutôt que créer 84 lignes `metadata` vides.
...
- [x] **trace-prod schema v25 — qc.reads_with_cpg** — 2 colonnes ajoutées (`reads_with_cpg`/`_pct`) + alimentation ponctuelle des 4 champs 28M/CpG sur 1332/1506 samples liquid (exclus : 147 solid + 27 sans `check-qc`), export gsheet 'QC read' (1362×16). Doc : nouvelle section README (table `qc` jamais documentée depuis v23) + CLAUDE.md v25 ; pas de checker permanent câblé (décision explicite).
- [x] **Fano (dispersion de couverture) — instruit sur 1 344 liquides** — 1re métrique du palier 1 à passer le critère de succès : 48 outliers Tukey sur le périmètre EPIC, dont **47 invisibles** aux seuils 5 M/0,25×, confondants CNV (−0,020) et longueur (+0,050) écartés. ⚠️ Le Fano génomique brut est inutilisable (satellites à 23 130×) et `corr(EPIC, génomique)` = +0,811 → l'apport réel du run EPIC reste à démontrer. Détails dans `fano-couverture-qc.md`.

- [x] **Onglet `Nb read mapped` — partie 3 remplie** — le placeholder « investigation en cours » laisse place à la distribution des populations 28M et CpG : 2 figures, tableau 7×8, bloc établi/non établi, titre renommé. La matrice est le seul axe discriminant (plasma 62,79 % / urine 52,74 %) et le ratio CpG/28M s'y inverse (71,20 vs 64,49) ; cancer/healthy ne sépare rien. Onglet 16 986 → 20 991 car.
...
- [x] **Lot Bladder_Blood intégré à la table `rarefaction`** — 269 pseudo-samples (58 bases × niveaux applicables) checkés en 2 passages puis exportés vers l'onglet `Rarefaction` (2962 lignes) ; `PROD` reste KO et seuls `mVAF v1.4` + `Props Bootstrap` sont complets (279/279), faute de `QC/` encore produit par Bam2Beta. 3 gotchas documentés dans CLAUDE.md (`e7ff4e6`) : les 2 vagues d'écriture de Bam2Beta, le faux zéro `nb_reads_total = 0,00`, et la cible = niveaux ≤ profondeur du parent (279 et non 58×5).
- [x] **Origine des reads non alignés des 16 urines de vessie** — deux populations sans recouvrement, séparées par le volume séquencé (3,3 M / 9,1 M) et non par le taux : 8 échantillons portent une bactérie qui occupe 52 à 87 % du séquençage (6 espèces différentes, aucune partagée, 2 co-infections), 8 n'ont aucun organisme dominant et relèvent d'un séquençage trop faible. Deux patients ont le même *Proteus mirabilis* mais des souches distinctes, ce qui exclut une contamination du laboratoire. L'origine du portage (infection / colonisation / souillure au recueil / prolifération avant congélation) n'est pas déterminable par le séquençage. Porté en Partie 5 du Google Doc QC + `memory/unmapped-reads-urines.md`.
- [x] **Référence d'alignement du pipeline identifiée** — MinKNOW 6.5.14 aligne en live sur `GRCh38_no_alt_analysis_set` (195 contigs, sans decoy hs38d1), et non sur le hg38 UCSC de `params.fasta` qui ne sert qu'aux modules. Vérifié par comparaison en-tête BAM ↔ `.fai`, 0 contig d'écart. Explique le non-alignement structurel du non-humain et la perte des reads humains courts par le preset `map-ont`. Porté en Architecture Notes.
- [x] **Finding `067` de covdepth requalifié** — les « 34 M reads alignés pour depth = 0 » de `Bladder_Urine_02_067` n'étaient pas des reads alignés : c'était `num_reads` de cramino, dont **99,82 % de non alignées**. La métrique `nb_reads_aligned` porte un nom faux, ce n'était pas une anomalie de mosdepth.
- [x] **Distribution du Primary mapped % — partie 5 du Google Doc** — médiane **73,25**, IQR 3,15 mais écart-type 7,39 : toute la dispersion vient d'une traîne basse (asymétrie −6,6, **112 aberrants bas et aucun haut**). Plafond **structurel** à ~80 %, le BAM portant toujours ~14 % de secondaires. Le seuil « > 97 % de mapping » de la littérature (PMID 37442577) est **réfuté** : max cohorte **96,42 %**, il rejetterait 1324/1324. L'urine de vessie concentre le signal (71,6 % d'aberrants contre 8,6 % pour le sang, à indication identique). Porté dans `memory/read-counting-cascade.md`.
- [x] **Architecture QC primaire / contributif — partie 6 du Google Doc** — deux niveaux séparés par une seule question : *un run supplémentaire corrigerait-il le défaut ?* **QC primaire** `nb_reads_aligned` (à renommer `nb_reads_primary`, A+D) **≥ 5 M** → arrêt, actionnable client ; **QC contributif** `reads_primary_mapped` (D) **≥ 4 M** → résultat rendu avec réserve. Seuils fondés sur la table `rarefaction` : **11,1 % de faux positifs induits** sous 5 M (458 échantillons aux 5 niveaux), coude à 4 M pour le contributif. 72 arrêts / 6 non contributifs / 1 246 contributifs. **Rien n'est implémenté.** Voir `memory/qc-deux-niveaux.md`.
- [x] **Le seuil de 5 M n'est appliqué automatiquement nulle part** — ni dans Bam2Beta ni dans trace-prod (grep exhaustif sur les 20 projets) : il n'existe que dans Aima-Tower (`callbacks.py:954`) sous forme de **ligne rouge sur boxplot**, et comme règle écrite dans `docs/QC-seuils-biopsie-liquide.md`. C'est une convention de lecture, pas une garde logicielle.
...
- [x] **Seuil de masse fixé à 22 % (et non 25 %)** — même méthode que pour le ratio : aucun plasma entre **18,07 et 25,94 %**, le milieu est 22 % avec ±3,9 de marge. Les deux valeurs classent les mêmes 16 échantillons, mais 25 % effleurait le bord haut de la vallée. Seconde bascule à **0,2 %** — les 12 contrôles externes y sont tous, contre 1 % des plasmas et aucun Twist.
- [x] **Seuil de longueur maintenu à 1 kb, argumenté** — instruit en recalculant ratio et masse à six seuils de 500 pb à 3 kb. Descendre coupe dans le cfDNA légitime (un plasma sain n'a que deux pics nucléosomaux, 160 et 296 pb) et **détruit la détection** : à 500 pb `Colon_22_rep1`, le plasma le plus altéré du jeu, retomberait en zone verte. Monter fait perdre le second critère : à 3 kb l'urine la plus chargée n'aurait plus que 2,5 % de masse. 1 kb ≈ 6 nucléosomes.
- [x] **Mécanismes des 3 plasmas de la zone grise tranchés par mesure directe** — critère : sur les reads ≥ 1 kb, part splittée et part de séquence alignée en continu. `Lung_Alc_93_av` = **chimères** (91,5 % splittées, 48 % continu, des fragments recollés) · `Lung_124` = **vrai ADN long** (2,7 % splittées, 99,3 % continu, jusqu'à 29 kb, même signature que `Breast_6`) · `Lung_Alc_15_av` indécidable (16 reads) et déjà rejeté. Ne pas conclure sur le seul `align/read`, qui n'est qu'un proxy.
- [x] **Piège du périmètre de mesure identifié** — trois chiffres du Google Doc venaient d'un échantillonnage sur `chr2:50-56 Mb` alors que tout le reste porte sur `chr1-22` : 3,5 % de reads > 1 kb (réel **3,02 %**), 63 % de masse (réel **57,26 %**), 57 105 pb de read maximale (réel **109 561 pb**). Règle : les mesures régionales qualifient un mécanisme, elles ne chiffrent jamais une proportion publiée. ⚠️ L'investigation des 4 plasmas HCL repose encore sur 2 Mb de `chr2`.
...
- [x] **Les 12 contrôles qualité externes forment un mode à part** — tous en zone grise, entre 1,3289 et 1,3649 (0,036 d'amplitude, le groupe le plus resserré de la cohorte) avec moins de 0,2 % de masse au-delà de 1 kb. Matériel de référence industriel, pas du plasma natif : leur position est attendue, ce n'est pas une alerte. Résout l'anomalie des « 13 plasmas de cause inconnue » — la zone grise se décompose en 16 urines + 12 EQC + **3 plasmas** (le 4e était le rebasecall de `Breast_17`, donc un EQC).
- [x] **Angle mort du ratio identifié** — calculé après retrait des reads > 1 kb, il ne peut pas signaler la contamination qu'il vient de filtrer : `Breast_6` (57 % de masse longue) et `TNE_2` (81 %) sont classés analysables. Le ratio ne doit pas être utilisé seul, mais accompagné de `pct_mass_removed` (~2 % chez un plasma normal, examiner au-delà de 25 %).
- [x] **Google Doc QC réécrit intégralement** — 9 sections, 5 figures, 5 tableaux, 3 listes nominatives (~16 400 car.). Ouvre sur le constat `Breast_6` au lieu d'une justification préalable, paragraphes courts + puces. Nouvelle section 8 « qui tombe hors de la zone verte » : un tableau par zone, plus l'application en aveugle sur les **10 patients Imagenome Labosud** (tous en zone verte) et les 12 contrôles externes.
- [x] **Erreur corrigée : `Colon_22` HCL et CGFL ne sont pas le même patient** — 75 noms d'échantillons sont partagés entre les deux labos et désignent des prélèvements différents. L'argument pré-analytique qui en découlait a été retiré du Google Doc. Toute jointure doit se faire sur le triplet `(nom, type, labo)`, faute de quoi les effectifs gonflent (1 393 lignes pour 1 243 plasmas).
...
- [x] **Google Doc QC — figures 1-2 passées en logique autosomes** — les deux figures reposaient sur `chr2:50-56 Mb` (12-17 k reads), refaites sur chr1-22 entiers (5,9-7,9 M reads) après vérification de comparabilité (écart moyen 0,16-0,24 point de masse, max 1,91 → la région était représentative). Le choix des autosomes n'est pas cosmétique : il aligne les figures 1-2 sur les données `read_lengths.csv` des parties 5-6, la partie 5 annonçant 57 % là où la mesure autosomes donne 57,5 % (tout-génome : 55,9 %). 3 valeurs du texte harmonisées, dont `63 % → 57 %` qui lève une incohérence interne préexistante entre les parties 1 et 5. Détails dans `gdoc-qc-ratio-n50.md`.

- [x] **trace-prod schema v22 — N75 + ratio N50/N75** — `qc_metrics.n75` (stocké mais jamais exporté) + `n50_n75_ratio` DECIMAL(10,4) exporté sous `Ratio N50/N75`, 3 combos, backfill 1471/1471, 0 incohérence au contrôle SQL. Gotcha retenu : `float('NaN')`/`float('inf')` ne lèvent pas ValueError et `n50/inf = 0.0` est fini → valider les opérandes, jamais le résultat. Détails dans `project_schema_v22_n75_ratio.md`.
- [x] **trace-prod — métriques de longueur liquid basculées sur le bloc filtré** — N50/N75/ratio du liquid viennent désormais de `n50_ratio.tsv` (reads > 1 kb écartés) via override `LiquidChecker` ; le solid reste sur cramino. 805 n50 modifiés sur 1 324, max 6010 → 574, médiane inchangée (174). ⚠ `N50` a désormais deux définitions selon l'onglet.
- [x] **trace-prod schema v24 — pct_mass_removed** — colonne `% Masse > 1kb` (liquid only) qui quantifie ce que le filtre retire : médiane 2,39 %, `TNE_2` 81,40 %. Répond directement à l'angle mort du ratio identifié côté Bam2Beta le même jour — les deux colonnes se lisent ensemble. Détails dans `project_schema_v24_pct_mass_removed.md`.

- [x] **Cascade de comptage des reads établie** — les 12 comptages du BAM merged définis et mesurés sur les 1 332 liquides, autour de 4 strates atomiques (A non alignées, B secondaires, C supplémentaires, D primaires mappées) dont la somme reboucle sur le Total pour **1 332/1 332**. Révèle que `nb_reads_aligned` du `metadata.json` porte un nom faux : il inclut les reads NON alignées (+12 % sur `Lung_9`, +22 % sur `Lung_Alc_79_av`). Détails dans `read-counting-cascade.md`.
- [x] **`idxstats` câblé dans Bam2Beta + rétrospectif 1 471 samples** — la strate des reads non alignées n'était mesurable nulle part (9,2 % sur `Lung_9`, jusqu'à 99,8 % sur certaines urines, ce qui explique enfin le cas `067` du chantier covdepth). Publié par `BAM_Count` (`c5e3765`) et les 1 471 samples de production déposés sur S3 en créations pures. Méthode **20× plus rapide** que le NFS : range request S3 sur les 3 premiers Mo du BAM + `.bai`. **`flagstat` écarté** — n'apporte rien, duplicates et QC-failed à 0 sur 10/10.
- [x] **Table `qc` trace-prod (schema v23)** — 25 colonnes, 12 comptages bruts + 11 pourcentages, alimentée pour les 1 332 liquides et exportée vers l'onglet `QC read`. `28M` et `MAPQ<20` créées mais NULL : `Preprocess_28M` tourne 22×/run sans publier de comptage. Commitée dans `dce1d45`.
- [x] **Google Doc QC — onglet « Nb reads mapped »** — 3 parties (définitions, cascade, anomalies), 13 titres, 5 figures, 2 tableaux. Chaque chiffre tracé à une commande, les non-mesurés signalés comme tels. Partie 3 : les 2 défauts **opposés** de `Lung_Alc_79_prog` (43 % de lignes supplémentaires) et `_av` (18 % de non-mapping) — le premier mappe mieux que son propre témoin — plus les 4 plasmas HCL dont le mécanisme reste **non tranché**.
- [x] **Figure de distribution de longueur livrée** — process `Length_Distribution_Plot` ajouté au workflow fragmentomique (`e8eac18`) : un PNG par sample superposant sa distribution à 3 références embarquées (plasma normal 0,31 % / urine 23,08 % / contamination gDNA 57,26 % de masse > 1 kb), avec le seuil 1 kb tracé. Pondéré par la **masse d'ADN** et non le nombre de reads, la médiane étant aveugle au défaut (163-176 pb chez les 8 plasmas contaminés quand le n50 monte à 3 647). Aucun nouveau conteneur (ggplot2 déjà dans `bam2beta:latest`), binning awk en streaming. Validation croisée : sur `Bladder_Urine_02_041`, le `pct_mass_removed` du run (23,08 %) est identique à la référence embarquée par deux chemins de calcul indépendants. Détails dans `length-distribution-figure.md`.
- [x] **MITO activé sur prod et solid** — `/test_bam2beta` sortait KO à l'étape 2 à chaque exécution, quel que soit le code testé : `check-run-output.sh` exige `mito_qc.tsv` dès que le sample est de type liquid, alors que le profil `prod` désactivait le module (`fca2844`). Le profil décidait de produire le fichier, le type de sample décidait de l'exiger. ⚠ Jamais exécuté sur du solid à ce jour.
...
- [x] **Refonte skill /deep-dive-concurency en 3 parties** — profil AIMA (`docs/AIMA-POSITIONING.md`, maj Exis 1.1 : mVAF v1.4 82%/95,1%) + extraction concurrent + comparaison ; template fiche concurrent + 1re fiche `NATERA-SIGNATERA-PROFIL.md` fact-checkée (test bout-en-bout, 5 erreurs corrigées avant rédaction). Mémoire : `aima_positioning_profil`, `signatera_natera`.
...
- [x] **Refonte de la procédure de qualif Bam2Beta** — `check-conformity.sh` refondu en 6 étapes (pattern valeur figée inline + 3 helpers d'extraction, non-régression **PROD vs QUALIF** sur valeurs natives ET `metadata.json` exhaustif hors champs volatils/contexte). **Lung_100 retiré** (qualif = Healthy_826 + Lung_9), `run-qualif.sh`/`test.sh` alignés. Testé sur run réel V2.2.1 → **54/54 CONFORME**, détection de régression prouvée. Commit `c1bd572`, détails [qualif-check-conformity.md](../../-home-blipinski-Pipeline-Bam2Beta/memory/qualif-check-conformity.md).
- [x] **Mode rétrospectif `--TOO_RETRO`** — score TOO depuis les 4 sorties déjà sur S3 (bootstrap props + Loyfer + raima_score.V1.4 + `IV/sex.tsv`), aucun recalcul, miroir strict de `--THEMELIO_RETRO`. Réutilise les process de `too.nf` tels quels, skip silencieux des samples incomplets. Lint +0 erreur, commit `7e7d90e`, doc CLAUDE.md à jour.
- [x] **Fix robustesse `--THEMELIO_RETRO`** — `checkIfExists: true` (qui tuait tout le run batch si un input manquait) remplacé par `.filter { .exists() }` + `log.warn` → skip silencieux. Commit `4d9fcdb`. Gotcha gravé en mémoire ; `--bootstrap`/`--METHYL_FEATURES` restent candidats au même fix.
