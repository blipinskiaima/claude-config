...
**Gotcha data (07/07/2026, PÉRIMÉ — voir « État final » ci-dessous) :** `raima_score.V1.3.tsv` **absent sur 50/50** → `mvaf_v13` toujours `NA`. `ichorCNA/` + `Fragmentomics/` sporadiques (Colon_8/Colon_27/Prostate_45…) → ichorcna/Mode1/Mode2/frag_score souvent NULL, se remplissent au fil du pipeline (UPSERT idempotent). C'était l'état **early-stage** du pipeline, pas une limite structurelle.
...
**Schema v17 (10/07/2026) — PK composite `(sample_name, labo)` :** l'ancienne PK `sample_name` seule causait une **collision inter-labo**. Un même pseudo-sample `{base}_{niveau}` peut exister dans `CGFL_rarefaction/` ET `HCL_rarefaction/` (18 bases communes observées : Colon_26/33/34/35/40/42/48/54/56/57/60/8, Lung_12/13, Pancreas_2/7/8/9 → 90 pseudo-samples). `check-rarefaction HCL` lancé après CGFL **écrasait** les 90 lignes CGFL (1355 checkés → 1265 en table). **Why:** dilution ne collisionne pas (`{Tumor}_{Healthy}_target` unique) donc le calque a hérité d'une PK trop faible pour rarefaction. **How to apply:** DDL `PRIMARY KEY (sample_name, labo)` + `labo NOT NULL` ; `upsert_rarefaction` force `labo` dans row + `ON CONFLICT (sample_name, labo)` + labo hors `DO UPDATE SET` ; migration idempotente v17 (`duckdb_constraints()` détecte PK mono-col → `rarefaction_new` DDL + `INSERT INTO SELECT` + DROP + RENAME, **jamais** CREATE AS SELECT) ; `update-column-rarefaction` ajoute `"labo": labo` aux 3 payloads (sinon INSERT sans labo viole NOT NULL). La migration **préserve** les lignes mais ne **restaure pas** les 90 CGFL écrasées → re-`check-rarefaction CGFL` post-migration. Backup `backup-pre-rarefaction-pk-v17-*.duckdb`, checkpoint `checkpoint-pre-rarefaction-pk-composite`.

Liens : [[project-schema-v9-dilution]] (pattern calqué, PK sample_name **suffisante** là-bas), [[project-schema-v8-short-read-metrics]] (« small_fragments »), [[project_columns_index]].
